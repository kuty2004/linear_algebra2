...
class TrainFaceData:
    ...
    def load_data_from_database(self):
        ...
        conn = mysql.connector.connect(
            ...
            user="root",       # 사용자 이름
            password="1234",   # 비밀번호
            database="face_recognition"  # 데이터베이스 이름
        )
        cursor = conn.cursor()

        # 쿼리 실행, "face_data" 테이블에서 데이터를 가져오는 SQL 쿼리
    ...
    def train(self):
        ...
        # 학습 세트의 각 사람을 루프
        for name, img_path in data:
            class_dir = name  # 클래스 "디렉토리 이름"을 SQL에서 가져온 "이름"으로 대체

            image = face_recognition.load_image_file(img_path) #img_path에서 이미지 로드
            ...
            num_train = int(len(valid_img_paths) * train_ratio)
            num_val = int(len(valid_img_paths) * val_ratio)
            ...
            for img_path in train_paths:
                ...
                if len(face_bounding_boxes) != 1:
                    ...
                    self.X_train.append(face_encoding)  # 여기서 수정
                    ...
                    for i in range(4): # 원본 이미지 + 증강 이미지 4개 추가
                        ...
                        self.X_train.append(aug_face_encoding)  # 여기서 수정
                        self.y_train.append(class_dir)
        ...
        if self.n_neighbors is None:
            self.n_neighbors = max(1, int(round(math.sqrt(len(self.X_train)))))
            if self.verbose:
...
class StudentAttendance:
    ...
    def attendanceupdate(past_names, current_names):
...
with open('/Users/yewon/Desktop/q/yongan_final_presentation/model_final.h5', 'rb') as f:
   knn_clf = pickle.load(f) 

# KNN 모델 학습
# train_data = TrainFaceData(model_save_path="model_35.h5", n_neighbors=None, knn_algo='ball_tree', verbose=False)
        ...
        for name in face_names:
            if name != "Unknown":
                ...
                if name not in face_recognition_times:
                    ...
                    StudentAttendance.attendanceupdate(past_names, current_names)

                    # 현재 얼굴 리스트와 과거 얼굴 리스트를 비교하면서
                    for i in range(length):
                        # 현재 얼굴이 과거 얼굴과 같은지 확인
                        if(len(current_names) != len(past_names)):
                            StudentAttendance.attendanceupdate(past_names, current_names)
